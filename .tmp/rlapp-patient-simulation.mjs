...
function buildSyntheticPatientProfile(index, width) {
    ...
    const secondaryName =
        secondaryNames[(index * 3 - 1) % secondaryNames.length];
...
function resolveDelayRange(prefix, defaultMin, defaultMax) {
    const minMs = readNonNegativeIntegerEnv(
        `RLAPP_MIN_${prefix}_MS`,
        defaultMin,
    );
    const maxMs = readNonNegativeIntegerEnv(
        `RLAPP_MAX_${prefix}_MS`,
        defaultMax,
    );
...
async function deactivateRoomAndRecord(room, processedPatients) {
    ...
    roomDeactivationDone =
        deactivatedRooms.length === targetDeactivatedRoomCount;
}
...
function buildCorrelationId(label) {
    const compactLabel =
        label.replace(/[^a-zA-Z0-9]/g, "").slice(0, 18) || "op";
...
async function callMedical(caseItem, room) {
    return runSerializedOperationalMutation(() =>
        request("/api/medical/call-next", {
            method: "POST",
            headers: headersFor(
                `medical-call-${caseItem.patientId}-${room.roomId}`,
            ),
            body: {
                queueId,
...
async function captureOperationalBaseline() {
    ...
    while (Date.now() < timeoutAt) {
        ...
        if (
            lastSnapshot &&
            dashboardCountersEqual(lastSnapshot, currentSnapshot)
        ) {
            if (stableSince === 0) {
    ...
    return {
        dashboard:
            lastSnapshot ?? extractDashboardCounters(await getDashboard()),
    };
}
...
async function processMedicalCase(caseItem, initialTurnId, baseTimings) {
    ...
    try {
        ...
        if (caseItem.category === "cancelled") {
            ...
            const trajectory = await pollTrajectory(
                caseItem,
                "TrayectoriaCancelada",
            );
...
async function runCashierStage(caseItem) {
    ...
    timings.cashierDwellMs = await waitForRandomDelay(
        timingProfile.cashierDwell,
    );
...
function buildSummary(results, operational, baseline) {
    ...
    return {
        ...
        monitor: {
            ...
            activeConsultationRooms:
                operational.monitor.activeConsultationRooms,
...
async function main() {
    ...
    const completedPatients = casePlan.filter(
        (item) =>
            ...
            item.category === "medicallyReviewed",
    ).length;
